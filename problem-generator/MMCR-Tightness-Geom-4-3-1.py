...
#Usage: MMCR-4-3-2.py maxCargo maxTightness numProblems numGeom pathToProblems
#
...
#Time windows all start from zero, thereby testing the concurrency of the 
#problems, as all delivery problems must be solved at once.
...
def main(args):
	...
	for p in range(0, numProblems):
		...
		sectors, locations, connectivityMap, pddl = MMCRProbGen.createSectors(numSectors, numLocations, capacityRange, False)
		...
		for seq in range(0,maxGeom):
			...
			pddl2 += MMCRProbGen.determineTimeWindows(deliveryInfo, connectivityMap, DEFAULT_TRAVEL_TIME, tightness, False, False)
